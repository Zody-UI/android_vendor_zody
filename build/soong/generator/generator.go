...
func init() {
	android.RegisterModuleType("zody_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = zodyExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = zodyExpandVariables(ctx, dep_file)
	...
	cmd := zodyExpandVariables(ctx, String(g.properties.Cmd))
